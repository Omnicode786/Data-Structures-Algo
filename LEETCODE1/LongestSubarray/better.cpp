...
int Betterusingmap(int array[], int size, int k) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        // ✅ 2️⃣ Agar sum khud hi k ke barabar ho gaya, toh full subarray valid hai
        if (sum == k) {
            maxLength = max(maxLength, i + 1); // Max length ko update karo
        }

        int rem = sum - k; // 🚀 3️⃣ Rem = sum - k nikal lo (ye past me exist hona chahiye)

        // ✅ 4️⃣ Dekho ki ye "rem" pehle kabhi aaya tha kya
        ...
        // ✅ 5️⃣ Sirf pehli baar aane wala sum store karna hai (pehli occurrence important hai!)
...
int main() {
    int a[5] = {1, 1, 2, 3, 2}; // Test case
    int n = Betterusingmap(a, 5, 2); // K = 2 ka longest subarray dhundo
